 ...
 */
function getTotalSum(arr: Sites[]): number {
	let sum = arr.reduce((prevSum, el) =>
		prevSum + el.siteCost ?? 0,
		0)
	return sum
}
let totalSum: number = getTotalSum(arrSites)
 ...
 */
function getSitesBetween(arr: Sites[]): number {
	let sum = arr.reduce((prevSum, el) =>
		(el.yearRelease > 2000 && el.yearRelease < 2009) ? prevSum + 1 : prevSum, 0)
	return sum
}
let sitesBetween: number = getSitesBetween(arrSites)
...
function getSumSponsorsMore100000(arr: Sites[]): number {
	let res: number = 0
	for (const el of arr) {
		let sum: number = 0
...
const sumSponsorsMore100000: number = getSumSponsorsMore100000(arrSites)
 ...
 */
function getAllSponsors(arr: Sites[]): sponsor[] {
	let res: sponsor[] = []
 ...
 */
function getYearMaxProfit(arr: Sites[]): string {
	let maxCost: number = 0, year: number = 0
 ...
 * @returns 
 */
function getSortListProfit(arr: Sites[]): number[] {
...
let copyArrSitesLess100000: Sites[] = [], copyArrSitesMore100000: Sites[] = []
for (const el of arrSites) {
